...
def app():
    ...
    download_themes_container = st.container()
    if st.session_state['theme_set'] is not None:
        with download_themes_container:
            st.write('## Download the theme set')
    ...
    for message in st.session_state["messages"]:
        if message["role"] == "user":
            if i != 0:
                ...
                st.chat_message("user").write(first_line)
        elif message["role"] == "assistant":
            st.chat_message("assistant").write(message["content"])
        i += 1

def display_theme_set_table(survey_data: pd.DataFrame):
    """
...
def handle_user_input(chat_input, survey_data):
    ...
    """
    st.write("DEBUG: Called handle_user_input")
    # with st.session_state['chat_container']:
    ...
    if assistant_message:
        ...
        add_message(assistant_message, 'assistant')
...
def generate_theme_set(user_input, survey_data):
    ...
    """
    st.write("DEBUG: Called generate_theme_set")
